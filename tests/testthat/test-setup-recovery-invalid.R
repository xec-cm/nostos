test_that("registration requires a nonempty TSE and unambiguous axis identities", {
  ...
  expect_setup_error(fixture, "tse", tse = matrix(1, 2L, 6L))
  se <- SummarizedExperiment::SummarizedExperiment(
    assays = list(counts = matrix(1, 2L, 6L))
  )
  expect_setup_error(fixture, "tse", tse = se)
  expect_setup_error(fixture, "tse", tse = fixture$tse[FALSE, ])
  expect_setup_error(fixture, "tse", tse = fixture$tse[, FALSE])

  for (axis in c("samples", "features")) {
    component <- switch(axis, samples = "sample IDs (colnames(tse))",
                        features = "feature IDs (rownames(tse))")
    for (bad_id in c("", " padded", "trailing ")) {
      ...
      expect_setup_error(bad, component)
    }
    bad <- registration_fixture()
    if (axis == "samples") {
  ...
  expect_setup_error(no_features, "feature IDs (rownames(tse))")
  colnames(fixture$tse) <- NULL
  expect_setup_error(fixture, "sample IDs (colnames(tse))")
})
...
test_that("analysis names and source selectors are explicit scalar names", {
  ...
  for (id in list(NULL, character(), NA_character_, "", "A", "a_b", "1a",
    ...
    expect_setup_error(fixture, "analysis_id", analysis_id = id)
  }
  for (selector in list(NULL, NA_character_, "", c("time", "batch"), 3L,
                        list("time"))) {
    expect_setup_error(fixture, "time_col", time_col = selector)
  }
  expect_setup_error(fixture, "source_columns", subject_col = "episode_id")
  expect_setup_error(fixture, "colData", episode_col = "missing")
  cd <- SummarizedExperiment::colData(fixture$tse)
  cd$second_time <- cd$time
  names(cd)[5] <- "time"
  SummarizedExperiment::colData(fixture$tse) <- cd
  expect_setup_error(fixture, "colData")
})
...
test_that("episode and event inputs require tables with unambiguous core columns", {
  ...
  for (bad_table in list(NULL, list(), matrix("e1", nrow = 1L))) {
    expect_setup_error(fixture, "episodes", episodes = bad_table)
    expect_setup_error(fixture, "events", events = bad_table)
  }
  for (component in c("episodes", "events")) {
    bad <- registration_fixture()
    bad[[component]] <- bad[[component]][FALSE, ]
    expect_setup_error(bad, if (component == "episodes") "samples" else "episodes$origin_event_id")
    bad <- registration_fixture()
...
test_that("core identity types and contents are validated without coercion", {
  fields <- list(
    c("episodes", "episode_id"), c("episodes", "subject_id"),
    c("episodes", "origin_event_id"), c("events", "event_id"), c("events", "episode_id")
  )
  for (field in fields) {
    component <- field[[1L]]
    column <- field[[2L]]
    invalid <- list(1:2)
    if (identical(field, c("episodes", "episode_id"))) {
      invalid <- c(invalid, list(list("first", "second"), c(NA, "valid"),
                                 c("", "valid"), c(" padded", "valid")))
    }
    for (bad_value in invalid) {
      bad <- registration_fixture()
      bad[[component]][[column]] <- bad_value
      expect_setup_error(bad, paste0(component, "$", column))
    }
  }
  for (column in c("subject_id", "episode_id")) {
    for (bad_value in list(1:6, as.list(rep("e1", 6L)),
      ...
      component <- switch(column, subject_id = "colData subject", episode_id = "colData episode")
      expect_setup_error(bad, component)
    }
  }
  for (bad_subject in c(NA_character_, "", " p1")) {
    ...
    SummarizedExperiment::colData(bad$tse) <- cd
    expect_setup_error(bad, "colData subject")
  }
})
...
test_that("explicit membership and origin references cannot be guessed or recycled", {
  ...
  expect_setup_error(bad, "episodes$episode_id")
  bad <- registration_fixture()
  bad$events$event_id <- c("ab1", "ab1")
  expect_setup_error(bad, "events$event_id")
  bad <- registration_fixture()
  bad$events$episode_id[1] <- "unknown"
  expect_setup_error(bad, "events")
  bad <- registration_fixture()
  bad$episodes$origin_event_id[1] <- "missing-event"
  expect_setup_error(bad, "episodes$origin_event_id")
  bad <- registration_fixture()
  ...
  for (boundary in c("Start", "sta", "", NA_character_)) {
    ...
    expect_setup_error(bad, "episodes$origin_boundary")
  }
  bad <- registration_fixture()
  bad$episodes$subject_id[1] <- "different-participant"
  expect_setup_error(bad, "samples")
  for (episode in c("unknown", "", " e1")) {
    bad <- registration_fixture()
    cd <- SummarizedExperiment::colData(bad$tse)
    cd$episode_id[1] <- episode
    SummarizedExperiment::colData(bad$tse) <- cd
    expect_setup_error(bad, if (episode == "unknown") "samples" else "colData episode")
  }
  bad <- registration_fixture()
  cd <- SummarizedExperiment::colData(bad$tse)
  cd$episode_id <- rep(NA_character_, 6L)
  SummarizedExperiment::colData(bad$tse) <- cd
  expect_setup_error(bad, "colData episode")
  bad <- registration_fixture()
  cd <- SummarizedExperiment::colData(bad$tse)
  cd$episode_id[4:5] <- "e1"
  SummarizedExperiment::colData(bad$tse) <- cd
  expect_setup_error(bad, "episodes")
})
...
test_that("time declarations require complete supported values", {
  ...
  for (unit in list(NULL, character(), NA_character_, "", "day", "Days", "d",
    ...
    expect_setup_error(fixture, "time_unit", time_unit = unit)
  }
  for (origin in list(NULL, character(), NA_character_, "", "   ",
                      c("origin1", "origin2"), 1L, list("enrolment"))) {
    expect_setup_error(fixture, "time_origin", time_origin = origin)
  }
})
...
test_that("time inputs are plain numeric vectors, never implicit calendar conversions", {
  ...
  for (times in invalid_times) {
    ...
    cd$time <- times
    SummarizedExperiment::colData(bad$tse) <- cd
    expect_setup_error(bad, "colData time")
    column <- "start_time"
    bad <- registration_fixture()
    if (is.matrix(times)) {
    ...
    expect_setup_error(bad, "events$start_time")
  }
  bad <- registration_fixture()
  bad$events$end_time <- c("14", "42")
  expect_setup_error(bad, "events$end_time")
})
...
test_that("included samples and event endpoints must have finite ordered times", {
  for (time in c(NA_real_, NaN, Inf, -Inf)) {
    ...
    for (column in c("start_time", "end_time")) {
      ...
      expect_setup_error(bad, paste0("events$", column))
    }
  }
  bad <- registration_fixture()
...
test_that("occupied names and reserved column prefixes reject repeated registration", {
  ...
  expect_setup_error(fixture, "analysis_id", class = "recoverome_error_collision")
  fixture <- registration_fixture()
  cd <- SummarizedExperiment::colData(fixture$tse)
  cd$rec_antibiotic_note <- "owned by the user"
  SummarizedExperiment::colData(fixture$tse) <- cd
  expect_setup_error(fixture, "colData", class = "recoverome_error_collision")
})
...
test_that("malformed or unsupported namespace containers are not overwritten", {
  ...
  expected_components <- c(rep("recoverome", 4L), rep("recoverome$analyses", 4L), "recoverome")
  for (i in seq_along(malformed)) {
    namespace <- malformed[[i]]
    bad <- registration_fixture()
    S4Vectors::metadata(bad$tse) <- list(recoverome = namespace, user_note = "keep")
    expect_setup_error(bad, expected_components[[i]], class = "recoverome_error_namespace")
  }
  bad <- registration_fixture()
  S4Vectors::metadata(bad$tse) <- setNames(list(NULL, list()), c("recoverome", "recoverome"))
  expect_setup_error(bad, "recoverome", class = "recoverome_error_namespace")
})
